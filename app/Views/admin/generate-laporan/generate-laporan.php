                     ...
                     <div class="col-md-6">
                        <div class="card h-100">
                           <form action="<?= base_url('admin/laporan/karyawan'); ?>" method="post" class="card-body d-flex flex-column">
                              <h4 class="text-primary"><b>Laporan Absen Karyawan</b></h4>
                              ...
                              <select name="jabatan" class="custom-select mt-3">
                                 <option value="">--Pilih jabatan--</option>
                                 <?php foreach ($jabatan as $key => $value) : ?>
                                    <?php
                                    $idJabatan = $value['id_jabatan'];
                                    $jabatan = "{$value['jabatan']} {$value['posisi']}";
                                    $jumlahKaryawan = count($karyawanPerJabatan[$key]);
                                    ?>
                                    <option value="<?= $idJabatan; ?>">
                                       <?= "$jabatan - {$jumlahKaryawan} karyawan"; ?>
                                    </option>
                                 <?php endforeach; ?>
                              </select>
